...
class Script extends bio.Struct {
  ...
  getSigops () {
    ...
    for (const op of this.code) {
      ...
      switch (op.value) {
        ...
        case opcodes.OP_CHECKMULTISIG:
        case opcodes.OP_CHECKMULTISIGVERIFY:
          if (lastOp >= opcodes.OP_1 && lastOp <= opcodes.OP_16) { total += lastOp - 0x50 } else { total += consensus.MAX_MULTISIG_PUBKEYS }
          break
      }
  ...
  static verify (witness, addr, tx, index, value, flags) {
    ...
    if (witness.items.length > consensus.MAX_SCRIPT_STACK) { throw new ScriptError('STACK_SIZE') }
    ...
    if (addr.version === 0) {
      if (addr.hash.length === 32) {
        ...
        if (witnessScript.length > consensus.MAX_SCRIPT_SIZE) { throw new ScriptError('SCRIPT_SIZE') }

        if (!sha3.digest(witnessScript).equals(addr.hash)) { throw new ScriptError('WITNESS_PROGRAM_MISMATCH') }
